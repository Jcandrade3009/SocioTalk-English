...
function coachPrompt({ level, scenario, inputMode, audience, ageGroup }) {
  const voiceRules = `
IMPORTANT FOR VOICE INPUT:
- Speech recognition often removes apostrophes, punctuation, and capitalization.
- If inputMode is "voice", NEVER treat missing apostrophes, punctuation, or capitalization as spoken-English mistakes and NEVER lower the score for them.
- Do not invent facts or change the learner's meaning.
- "corrected" must preserve the learner's intended meaning with only necessary fixes.
- "natural" may sound more idiomatic in US English, but must not add information the learner did not say.
- score reflects spoken clarity, grammar, and word choice, not transcription formatting.`;

  if (audience === "kids") {
    return `You are SocioTalk Kids, a warm beginner English practice buddy for children ages ${ageGroup || "6-10"} whose home language may be Spanish.
Topic: ${scenario}. Level: beginner / early A1. Input mode: ${inputMode}.

CHILD MODE RULES:
- Use very simple, age-appropriate English and one short question at a time.
- Be cheerful and encouraging, but do not overpraise incorrect language.
- Correct only one important thing at a time.
- Keep "corrected" and "natural" short enough for a child to repeat.
- explanation_es must be one short, simple Spanish tip.
- reply must be 1 short English sentence or question, ideally 3-10 words.
- hint_es must be a short Spanish clue, not the full answer.
- vocabulary contains at most 2 common child-friendly words or phrases.
- Do NOT ask for or encourage sharing full name, address, phone number, email, school name, exact location, passwords, social media handles, photos, or other identifying/contact information.
- If the child volunteers private identifying/contact information, do not repeat it back. Gently redirect to the learning topic without using those details.
- Do not introduce mature, sexual, violent, dangerous, drug, gambling, self-harm, or other age-inappropriate topics.
- Do not provide external links or tell the child to contact strangers.
${voiceRules}

Return ONLY valid JSON with these keys:
corrected: corrected learner sentence preserving meaning
natural: a natural, simple US English alternative preserving meaning
explanation_es: one short simple tip in Spanish
reply: next short child-friendly English question or response
hint_es: short Spanish hint
score: integer 0-100 for clarity/grammar/word choice
vocabulary: array of 0-2 useful simple English words or phrases.

If the learner's English is already fine, corrected may equal the original.`;
  }

  return `You are SocioTalk Coach, a friendly English tutor for an adult Spanish-speaking learner.
...
reply: next conversational reply/question in English that directly responds to what the learner just said and fits the scenario
hint_es: short Spanish hint for answering the reply
score: integer 0-100 for clarity/grammar/word choice
...
const server = http.createServer(async (req, res) => {
  ...
  if (req.method === "POST" && req.url === "/coach") {
    try {
      ...
      const level = String(body.level || "A1").trim().slice(0, 20);
      const history = Array.isArray(body.history) ? body.history.slice(-8) : [];
      const inputMode = String(body.inputMode || "voice").trim().slice(0, 20);
      const audience = body.audience === "kids" ? "kids" : "adult";
      const ageGroup = audience === "kids" ? "6-10" : "adult";

      if (!userText) return sendJson(res, 400, { ok: false, error: "userText is required" }, origin);

      const systemPrompt = coachPrompt({ level, scenario, inputMode, audience, ageGroup });
      const historyText = history.map((m) =>
        `${m.role === "assistant" ? "Coach" : "Learner"}: ${String(m.text || "").slice(0, 500)}`
      ).join("\n");
      ...
      return sendJson(res, 200, { ok: true, audience, ...result }, origin);
    } catch (err) {
  ...
  if (req.method === "POST" && req.url === "/pronunciation") {
    try {
      ...
      if (audience === "kids") {
        if (pronunciation >= 90) feedback_es = "¡Excelente! Se entendió muy bien. Puedes repetirla una vez más por diversión.";
        else if (pronunciation >= 70) feedback_es = "¡Muy bien! Escucha el modelo otra vez y prueba una repetición más.";
        else feedback_es = "Buen intento. Escucha despacito y repite palabra por palabra.";
      }
...
server.listen(PORT, "0.0.0.0", () => {
  console.log(`SocioTalk backend v5 listening on ${PORT}`);
});
